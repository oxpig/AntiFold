...
class PDBNumber(object):
    ...
    def numberSequence(self, sequenceName, sequence):
        ...
        if len(sequence) > 70:
            ...
            # results = _anarci( [("sequence_0", sequence)], scheme="imgt", database="ALL", output=False, allow=allow, allowed_species=allowed_species)
        else:
            return
...
class SelectFv(Select):
    def accept_residue(self, residue):
        if "anarci_details" in residue.parent.xtra:
            if int(residue.id[1]) > 0 and int(residue.id[1]) < 1000:
                return 1
